...
sock.connect((MC_SERVER_IP, int(MC_RCON_PORT)))

mcrcon = MCRcon(MC_SERVER_IP, MC_RCON_PASS)
if not mcrcon:
...
def check_difficulty():
    difficulty = mcrcon.command(sock, 'difficulty')
    print(difficulty)
...
def change_difficulty(difficulty):
    ...
    mcrcon.command(sock, "difficulty {}".format(difficulty))

@bot.command(name='info', help='Lists all pending proposals')
async def info(ctx):
    if GUILD == str(ctx.guild) or GUILD == 'Test Minecraft Server':
...
async def command(ctx, *args):
    command = " ".join(args[:])
    resp = mcrcon.command(sock, command)
    await ctx.send(resp)

@bot.command(name='zzz', help='Sets time to day if night')
@commands.has_role('server_admin')
async def zzz(ctx):
    time_data = mcrcon.command(sock, 'time')
    ...
    if time_data < 600 or time_data > 2000:
        ...
        mcrcon.command(sock, 'time set day')
